...
and does not reside in a range of slink:VkDeviceMemory bound to the resource.
...
that is, it is independent of any modifications implementation-private memory
associated with the resource that is not contained in its slink:VkDeviceMemory.
...
This flexibility simplifies the implementation of layers that provide Vulkan
WSI (such as link:{refpage}vkCreateWaylandSurfaceKHR.html[vkCreateWaylandSurfaceKHR],
link:{refpage}VkSwapchainKHR.html[VkSwapchainKHR], and
link:{refpage}vkQueuePresentKHR.html[vkQueuePresentKHR])
by implementing it atop
link:{refpage}VK_KHR_external_memory.html[VK_KHR_external_memory]
and native window system APIs.
